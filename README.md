...
Geplant ist Go mit `net/http` und `github.com/go-chi/chi/v5`.
...
Geplant ist `github.com/go-playground/validator/v10`.
...
Geplant ist `gorm.io/gorm` mit `gorm.io/driver/postgres`.

GORM soll als OR-Mapper fuer PostgreSQL verwendet werden. Dadurch koennen Entitaeten als Go-Structs modelliert und Datenbankoperationen wie Lesen und Neuanlegen uebersichtlich umgesetzt werden.

### Optional: OIDC mit Keycloak

Keycloak soll als OIDC-Provider eingebunden werden.
...
Die REST-Schnittstelle kann damit ueber Bearer Tokens abgesichert werden. Der Server prueft dabei das vom Client mitgesendete Access Token gegen die Keycloak-Konfiguration, insbesondere Issuer, Client-ID/Audience und Signatur.
