...
func ResetAndPopulate(db *gorm.DB) error {
	return db.Transaction(func(tx *gorm.DB) error {
		statements := []string{
			`TRUNCATE TABLE soldat.verletzung, soldat.ausruestung, soldat.soldat RESTART IDENTITY CASCADE`,
			`INSERT INTO soldat.soldat (version, vorname, nachname, geburtsdatum, geschlecht, rang, username, erzeugt, aktualisiert)
			 VALUES
			...
			`INSERT INTO soldat.ausruestung (waffe, seriennummer, soldat_id)
			 VALUES
			...
			`INSERT INTO soldat.verletzung (verletzungsbezeichnung, behandelt, schweregrad, verletzungsdatum, soldat_id)
			 VALUES
